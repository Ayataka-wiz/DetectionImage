...
def main():
    ...
    st.title("Similar anime characters")
    ...
    if uploaded_file is not None:
        ...
        #pred2 = round(prediction[2],3) * 100  # あざらしの確率(%)
        ...
        #else:
        #    st.subheader(f"これは{pred2}％の確率で「あざらし」です！")
        

        # 推論の確率を小数点以下3桁で丸め×100(%に変換)
        pred0 = round(prediction[0],3) * 100  # 戸塚の確率(%)
        pred1 = round(prediction[1],3) * 100  # アナスタシアの確率(%)
        # 推論で得られたクラス番号(初期値は0)によって出力結果を分岐
        if classNo == 0:
            st.subheader(f"これは{pred0}％の確率で「戸塚！！！！！！！！」です！")
        else:
            st.subheader(f"これは{pred1}％の確率で「ふ～ん、アナスタシアやん。デレステやん。」です！")
...
# 学科：情報システム科
# 学年：２年
# コメント：タイトル「似すぎな男女のアニメキャラクター」
#          前々から似ている二人だと思い、認識AIにやらせてみたらどうなるかおもしろそうだと思いこれにしました。
#
#=============================================================================#
